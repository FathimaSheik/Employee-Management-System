...
function Employee(name, age, department, salary) {
  ...
  function calculateAverageSalary() {
    ...
    displayOutput(`Average Salary: $${averageSalary.toFixed(2)}`, "Arial","red", "bold");
  }
  
  // Find Employees in a Department
  function findEmployeesInDepartment(department) {
    const deptEmployees = employees.filter(emp => emp.department === department);
    displayOutput(`Employees in ${department}: ${deptEmployees.map(emp => emp.name).join(', ')}`, "Arial", "red", "bold");
  }
  ...
  function increaseSalary(percentage) {
    ...
    displayOutput(`Salaries increased by ${percentage}%.`,"Arial", "red", "bold");
  }
  
  // Sort Employees by Age
  function sortEmployeesByAge() {
    const sortedEmployees = employees.slice().sort((a, b) => a.age - b.age);
    displayOutput(`Sorted Employees by Age: ${sortedEmployees.map(emp => emp.name).join(', ')}`, "Arial", "red", "bold");
  }
  ...
  function displayAllEmployees(){
    ...
    employees.forEach(emp => {
        ...
        displayOutput(`${outputDiv.innerHTML} ${employeeInfo}`,"Arial","blue", "bold");
    });
  }
  // Display output in HTML
  function displayOutput(message,fontFamily, color, fontWeight) {
    document.getElementById('output').innerHTML = `<p style="font-family: ${fontFamily}; color: ${color}; font-weight: ${fontWeight}">${message}</p>`;
  }  
